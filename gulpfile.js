    ...
    .on('end', () => {
      ...
      buildJS()
        ...
        .on('end', () => {
          ...
          g.util.log(`BUILD REACT MINI TOOLS PAGE FILES => ${new Date().toLocaleString()}`);
        });
    });
};
...
const copyAssets = () => {
  gulp
    ...
    .pipe(
      gulp.dest(environment === 'production' ? `${destPath}/images` : `${destPath}/react-mini-tools/images`)
    );
};
...
gulp.task('build-production', () => {
  ...
  destPath = './../website-router/public/react-mini-tools';
  buildHTML();
  copyAssets();
});
